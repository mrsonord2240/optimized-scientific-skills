...
- `mageck mle`'s primary `fdr` column is permutation-based and its granularity depends on `--permutation-round` (default 2, `--help` suggests 10). At the default, even identical reruns can flip a boundary call because this MAGeCK version has no `mle --seed`; state a higher round count for results you report, rerun borderline calls, and cross-check `wald-fdr`. The effect size is screen-dependent, so do not generalize one run's flip count.
- For a pure two-condition matched-pair experiment, use `mageck test --paired` with treatment and control samples in matching donor order. Use MLE with donor/condition columns only when pairing must coexist with further factors such as dose, time, or batch.
- In `FluteMLE`, `ctrlname` and `treatname` must be actual emitted MLE condition names (for example `day7` and `day21`), never the implicit all-ones `baseline` intercept.
