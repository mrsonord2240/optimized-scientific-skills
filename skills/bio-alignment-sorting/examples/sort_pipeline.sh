...
if [ -z "$REF" ] || [ -z "$R1" ] || [ -z "$OUTPUT" ] || [ "$#" -gt 5 ]; then
    ...
    echo "Example (single): sort_pipeline.sh ref.fa reads.fq aligned.bam [threads]"
    exit 1
fi
...
# Publish only after the count and index checks succeed. This avoids a partial
# BAM at OUTPUT and preserves an existing output if this invocation fails.
TMP_OUTPUT="${OUTPUT}.partial.$$"
cleanup() {
    rc=$?
    rm -f "$TMP_OUTPUT" "${TMP_OUTPUT}.bai"
    exit "$rc"
}
trap cleanup EXIT
...
if [ -z "$R2" ]; then
    ...
    bwa mem -t "$THREADS" "$REF" "$R1" | samtools sort -@ 4 -o "$TMP_OUTPUT"
    EXPECTED=$(( $(gzip -cdf "$R1" | wc -l) / 4 ))
else
    # Paired-end
    bwa mem -t "$THREADS" "$REF" "$R1" "$R2" | samtools sort -@ 4 -o "$TMP_OUTPUT"
...
GOT=$(samtools view -c -F 0x900 "$TMP_OUTPUT")
if [ "$GOT" -ne "$EXPECTED" ]; then
    echo "ERROR: alignment has $GOT primary records, expected $EXPECTED input reads; output was not published" >&2
    exit 1
fi
...
mv -f "${TMP_OUTPUT}.bai" "${OUTPUT}.bai"
trap - EXIT
